...
const Home = () => {
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    const credentials = {
      Admin: { user: "Admin", pass: "Admin@123", redirect: "/admin/home" },
      Teacher: { user: "Teacher", pass: "Teacher@123", redirect: "/teachers/home" },
      Student: { user: "Student", pass: "Student@123", redirect: "/student/home" },
    };

    if (username === credentials[role].user && password === credentials[role].pass) {
      toast.success(`${role} logged in successfully!`, { position: "top-right", autoClose: 3000 });
      setTimeout(() => navigate(credentials[role].redirect), 1000);
    } else {
      toast.error(`Invalid ${role} username or password!`, { position: "top-right", autoClose: 3000 });
    }
  };
          ...
          <div className="w-full md:w-1/2 p-6 sm:p-8 bg-[url('https://t3.ftcdn.net/jpg/03/55/60/70/360_F_355607062_zYMS8jaz4SfoykpWz5oViRVKL32IabTP.jpg')] bg-cover bg-center">
            ...
            <form onSubmit={handleLogin} className="space-y-3">
              ...
              <div>
                ...
                />
                {username && (
                  <p className={`text-sm mt-1 ${usernameMsg.includes("✅") ? "text-green-600" : "text-red-600"}`}>
                    {usernameMsg}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                />
                {password && (
                  <p className={`text-sm mt-1 ${passwordMsg.includes("✅") ? "text-green-600" : "text-red-600"}`}>
                    {passwordMsg}
                  </p>
                )}
              </div>
